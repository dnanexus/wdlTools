...
import wdlTools.syntax.v1_1.{ConcreteSyntax => CST}
...
case class ParseAll(followImports: Boolean = false,
  ...
  private val versionRegexp = "version\\s+1\\.1.*".r
  ...
  override def parseDocument(fileSource: FileNode): AST.Document = {
    val grammar = WdlV1_1Grammar.newInstance(fileSource, listenerFactories, logger)
  ...
  override def parseExpr(text: String): AST.Expr = {
    ...
    val parser = ParseTop(WdlV1_1Grammar.newInstance(docSource, listenerFactories, logger))
