...
object ConcreteSyntax {
  ...
  case class TypeStruct(name: String,
                        ...
                        text : TextSource) extends Type with DocumentElement
  ...
  // true="--yes"    false="--no"
  case class ExprPlaceholderPartEqual(b: Boolean, value: Expr, text : TextSource) extends PlaceHolderPart
  // default="foo"
  ...
  case class Declaration(name: String, wdlType: Type, expr: Option[Expr], text : TextSource) extends WorkflowElement
  ...
  case class ImportDoc(name: Option[String], aliases: Vector[ImportAlias], url: URL, text : TextSource)
  ...
  case class CallAlias(name: String,
                       text : TextSource) extends Element
  case class CallInput(name: String,
                       expr: Expr,
                       text : TextSource) extends Element
  case class CallInputs(value: Map[String, Expr],
                        text : TextSource) extends Element
  case class Call(name: String,
                  alias: Option[String],
                  inputs: Map[String, Expr],
                  text : TextSource) extends WorkflowElement
