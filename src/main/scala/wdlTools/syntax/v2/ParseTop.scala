...
import org.openwdl.wdl.parser.v2.{WdlV2Lexer, WdlV2Parser, WdlV2ParserBaseVisitor}
...
case class ParseTop(opts: Options,
                    grammar: Grammar[WdlV2Lexer, WdlV2Parser],
                    docSourceUrl: Option[URL] = None)
    extends WdlV2ParserBaseVisitor[Element] {
...
any_decls
  ...
  override def visitTask_runtime_kv(ctx: WdlV2Parser.Task_runtime_kvContext): RuntimeKV = {
    val id: String = Vector(
      ...
      case Some(id) => id
      case _ =>
        throw new SyntaxException(s"invalid runtime keyword", getTextSource(ctx), docSourceUrl)
    }
    val expr: Expr = visitExpr(ctx.expr())
