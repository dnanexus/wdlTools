...
import wdlTools.syntax.{AbstractSyntaxTreeVisitor, VisitorContext, WdlVersion}
import wdlTools.syntax.AbstractSyntax._
import wdlTools.util.{FileSourceResolver, FileUtils, Logger}

case class RuleContext(wdlVersion: WdlVersion,
                       fileResolver: FileSourceResolver = FileSourceResolver.get,
                       logger: Logger = Logger.get) {
  def getWdlFilename(uri: String, removeSuffix: Boolean = false): String = {
    val fileName = fileResolver.resolve(uri).fileName
    if (removeSuffix) {
...
object AbstractSyntaxTreeRules {
  class LinterAstRule(conf: RuleConf) extends AbstractSyntaxTreeVisitor {
    ...
    protected def addEventFromElement(element: Element, message: Option[String] = None): Unit = {
      events :+= LintEvent(conf, element.loc, message)
    }
  }

  type LinterAstRuleApplySig = (RuleConf, RuleContext) => LinterAstRule
  ...
  case class NonPortableTaskRule(conf: RuleConf, ruleCtx: RuleContext) extends LinterAstRule(conf) {
  ...
  case class NoTaskInputsRule(conf: RuleConf, ruleCtx: RuleContext) extends LinterAstRule(conf) {
    override def visitTask(ctx: VisitorContext[Task]): Unit = {
  ...
  case class NoTaskOutputsRule(conf: RuleConf, ruleCtx: RuleContext) extends LinterAstRule(conf) {
    override def visitTask(ctx: VisitorContext[Task]): Unit = {
  ...
  /**
    ...
    */
  case class NameCollisionRule(conf: RuleConf, ruleCtx: RuleContext) extends LinterAstRule(conf) {
  ...
  /**
    ...
    */
  case class UnusedImportRule(conf: RuleConf, ruleCtx: RuleContext) extends LinterAstRule(conf) {
    ...
    override def visitDocument(ctx: VisitorContext[Document]): Unit = {
      ...
      val allImports: Map[String, ImportDoc] = ctx.element.elements.collect {
        case imp: ImportDoc =>
          imp.name
            .map(_.value)
            .getOrElse(ruleCtx.getWdlFilename(imp.addr.value, removeSuffix = true)) -> imp
      }.toMap
  ...
  /**
    ...
    */
  case class UnknownRuntimeKeyRule(conf: RuleConf, ruleCtx: RuleContext)
      extends LinterAstRule(conf) {

    // TODO: move to built-ins
    private val keys = Set(
        ...
        "maxRetries",
        "returnCodes"
    ) ++ (if (ruleCtx.wdlVersion <= WdlVersion.V1) {
            Set(
                "bootDiskSizeGb",
  ...
  /**
    ...
    */
  case class MissingVersionRule(conf: RuleConf, ruleCtx: RuleContext) extends LinterAstRule(conf) {
    override def visitDocument(ctx: VisitorContext[Document]): Unit = {
      if (ruleCtx.wdlVersion < WdlVersion.V1) {
        addEvent(ctx)
      }
