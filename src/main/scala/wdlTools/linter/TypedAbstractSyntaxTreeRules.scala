...
object TypedAbstractSyntaxTreeRules {
  class LinterTstRule(conf: RuleConf) extends TypedAbstractSyntaxTreeVisitor {
    ...
    protected def addEventFromElement(element: Element, message: Option[String] = None): Unit = {
      addEventFromSource(element.loc, message)
    }

    protected def addEventFromSource(loc: SourceLocation, message: Option[String] = None): Unit = {
    ...
    override def visitCommandSection(ctx: VisitorContext[CommandSection]): Unit = {
      ...
      if (ShellCheckRule.shellCheckAvailable.get) {
        ...
        def callback(expr: Expr): Option[String] = {
          @tailrec
          def getDummyVal(t: T, textSource: SourceLocation): Option[String] = {
            t match {
        ...
        try {
          ...
          val commandStart = ctx.element.parts.head.loc
          shellCheckCommand.!!.parseJson match {
            case JsArray(elements) =>
              elements.foreach { lint =>
                ...
                addEventFromSource(SourceLocation(
                                       docLine,
                                       docCol,
                                       docLine,
                                       docCol
